...
public class UserActions {
    ...
    public void mainMenu(MenuPresenter menuPresenter, UserManager userManager, TradeCreator tradeCreator, User user){
        ...
        while (running) {
            ...
            menuPresenter.printMenu(16, 0); // User Menu
            menuPresenter.printMenu(16, 1); // (1) View items and wishlist
            menuPresenter.printMenu(16, 2); // (2) View user stats
            menuPresenter.printMenu(16, 3); // (3) Request an unfreeze
            menuPresenter.printMenu(16, 4); // (4) View other users
            menuPresenter.printMenu(16, 5); // (5) View your pending trades
            menuPresenter.printMenu(16, 6); // (6) View active temporary trades
            menuPresenter.printMenu(16, 7); // (0) Quit
    ...
    public void viewItemAndWishlist(MenuPresenter menuPresenter, UserManager userManager, User user){
        ...
        if (user.getAvailableItems().size() == 0){
            ...
            menuPresenter.printMenu(17, 2);
            for (Item item : user.getAvailableItems()) {
                System.out.println(item);
            }
        }
        // System.out.println("\n Your wishlist :");
        menuPresenter.printMenu(17, 3);
    ...
    public void runStats(MenuPresenter menuPresenter, UserManager userManager, TradeCreator tradeCreator, User user) {
        ...
        while (!handled) {
            while (true) {
                Scanner scan = new Scanner(System.in);
                menuPresenter.printMenu(21, 0); // View User Stats
                menuPresenter.printMenu(21, 1); //(1) View number of items user has borrowed
                menuPresenter.printMenu(21, 2); // (2) View number of items user has lent
                menuPresenter.printMenu(21, 3); // (3) View frozen status
                menuPresenter.printMenu(21, 4); // (4) View number of trades involving user that have not been completed
                menuPresenter.printMenu(21, 5); // (5) View number of transactions this week
                menuPresenter.printMenu(21, 6); // (6) View items recently traded away
                menuPresenter.printMenu(21, 7); // (7) View most frequent trading partner's
                menuPresenter.printMenu(21, 8); // (8) Return to User Menu
                input = scan.nextInt();
                if (input > 8 || input < 1) {
                    // System.out.println("Please enter a number from 1 to 8");
                    menuPresenter.printMenu(21, 9);
                } else break;
            }
            if (input == 1) {
                // System.out.println("You have borrowed " + Integer.toString(user.getNumBorrowed()) + " items.");
                menuPresenter.printMenu(21, 10);

            } else if (input == 2) {
                // System.out.println("You have lent " + Integer.toString(user.getNumLent()) + " items.");
                menuPresenter.printMenu(21, 11);

            } else if (input == 3) {
                if (user.getFrozen()) {
                    // System.out.println("Your account has been frozen");
                    menuPresenter.printMenu(21, 12);
                } else {
                    // System.out.println("Your account is not frozen");
                    menuPresenter.printMenu(21, 13);
                }
    ...
    public void viewPendingTrades(MenuPresenter menuPresenter, TradeCreator tradeCreator, User user){
        ...
        while (choice != 0 ) {
            ...
            menuPresenter.printMenu(23, 1);
            // System.out.println("(1) Exit this menu");
            menuPresenter.printMenu(23, 2);
            // System.out.println("====================");
            menuPresenter.printMenu(23, 3);
            // System.out.println("Your pending trades:");
            menuPresenter.printMenu(23, 4);
